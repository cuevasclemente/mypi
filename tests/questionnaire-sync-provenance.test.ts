...
import questionnaire from "../plugins/questionnaire.ts";
...
async function executeWebQuestionnaire(outcome: unknown) {
	let tool: any;
	questionnaire({
...
test("synchronous Wayang submissions expose durable request and submission provenance", async () => {
	const result = await executeWebQuestionnaire({
		status: "submitted",
	...
	assert.equal(
		...
		"Questionnaire submitted (request ID \"request-123\", submission ID \"submission-456\\ninjected\").\n" +
			"Q1: user selected: 1. Option A",
	);
});

test("a submitted bridge outcome without server submission provenance fails closed", async () => {
	const result = await executeWebQuestionnaire({
		status: "submitted",
		request: { requestId: "request-123" },
		answers: [{ id: "scope", value: "a", label: "Option A", wasCustom: false, index: 1 }],
...
test("pending Wayang questionnaires retain their existing request-only contract", async () => {
	const result = await executeWebQuestionnaire({
		status: "pending",
		request: { requestId: "request-123" },
	});
	...
	assert.equal(
		result.content[0].text,
		"The questionnaire remains open (request request-123). Do not treat this as cancelled; " +
